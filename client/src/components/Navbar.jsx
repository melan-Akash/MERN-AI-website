...
import { ArrowRight, LogOut, ChevronDown, Sparkles } from 'lucide-react'
  ...
  return (
    <div className='fixed z-50 w-full backdrop-blur-2xl flex justify-between items-center py-4 px-4 sm:px-20 xl:px-32 border-b border-white/10'>
      <div onClick={() => navigate('/')} className='flex items-center gap-2 cursor-pointer'>
        <Sparkles className='w-7 h-7 text-[#5044E5] fill-[#5044E5]' />
          ...
          >
            {user.imageUrl ? (
              ...
              />
               ...
               <div className='w-8 h-8 rounded-full bg-[#5044E5] text-white flex items-center justify-center font-bold text-xs'>
                 {user.fullName?.charAt(0)?.toUpperCase()}
               </div>
            )}
            <span className='hidden sm:block text-white font-medium text-sm'>{user.fullName}</span>
